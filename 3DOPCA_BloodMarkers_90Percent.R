...
for (sexLoop in 1:2){
  ...
  for (bMarkLoop in 1:7){
    ...
    while(goodSet < 1){
      ...
      for (pp in 1:nFolds){ #If any fold has no extreme cases, then reshuffles
        if (sum(dataCheck[which(dataCheck$predDataFolds == pp),1]) == 0){
          goodSet <- 0
        }
        if (sum(dataCheck[which(dataCheck$predDataFolds == pp),1]) == length(dataCheck[which(dataCheck$predDataFolds == pp),1])){
          goodSet <- 0
        }
      }
